...
* [Deploying new load modules and performing a NEWCOPY on the related programs](#newcopy)
...
![Diagram showing a process with Copy Artifacts,Deploy Data Sets, and New copy resources steps.](ucd_cicsts_doc_scenario_process22.png?resize=241%2C364)Diagram showing a process with Copy Artifacts,Deploy Data Sets, and New copy resources steps 


### Deploying and undeploying CICS applications


* [Component and component version for a CICS application](#comp)
* [Component process to deploy a CICS application](#deploy)
* [Component process to undeploy a CICS application](#undeploy)
...
![Image showing the default values for the Deploy application step](deploy_app.png?resize=541%2C294)Image showing the default values for the Deploy application step
...
![default values for the Deploy application step](undeploy_app.png?resize=573%2C509)Image showing the default values for theUndeploy application step
#### Required properties
...
Secondly, using the same steps, configure a key ring and certificate for the agents userid. Alternatively, you can use JKS or PKCS12 format keystores for the agent. For example usage, see [Configuring SSL in the CICS TS plug-in](#ucd).


Finally, in order to complete the handshake, connect the CA certificate of the CICS region to the agent userskey ring, and the CA certificate of the agent user to the CICS region’s key ring. This can be done using the RACDCERT CONNECT command, with USAGE(CERTAUTH).
